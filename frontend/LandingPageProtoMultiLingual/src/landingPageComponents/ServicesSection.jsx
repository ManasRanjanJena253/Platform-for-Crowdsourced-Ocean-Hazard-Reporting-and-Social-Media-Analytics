...
  ];
  ...
  // 2. The servicesData object is now built inside the component using the 't' function
  const servicesData = {
    citizens: {
  ...
  const listContainerVariants = { /* ... remains the same ... */ };
...
      </div>
      ...
      <div className="relative z-10 container mx-auto max-w-6xl text-center">
        {/* 3. Main title and subtitle are now translated */}
        ...
        <div className="flex justify-center space-x-2 md:space-x-8 mb-10 border-b border-gray-200">
          {Object.keys(servicesData).map((key) => (
            ...
            >
              {/* 4. Tab button labels are now translated */}
              {t(`services.tabs.${key}`)}
              {activeTab === key && (
                <motion.div
